...
distr <- Abalone$age
hist(distr, main= "Distribution of age", xlab= "Abalone age", ylim= c(0,1500), xlim= c(0,35), col= "darksalmon", yaxt= "n")
...
Abalone_recipe <- recipe(age ~ M + X0.455 + X0.365 + X0.095 + X0.514 + X0.2245 + X0.101 + X0.15, data= Abalone_train)
...
The variable rings is proportional to the age of the abalone, however taking it into consideration to predict the age of the abalone can lead to overfitting.
...
FitModel <- lm(age ~ M + X0.455+ X0.365+ X0.095+ X0.514+ X0.2245 + X0.15, data= Abalone)
summary(FitModel)

Hypothetical <- data.frame(M= c('F'), X0.455= c(0.50), X0.365= c(0.10), X0.095= c(0.30), X0.514= c(4), X0.2245= c(1), X0.101= c(2), X0.15= c(1))
